...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	for (;;)
	{
		...
		for (;;)
		{
			cameraImage = cv::imread("cameraImg.png", CV_LOAD_IMAGE_GRAYSCALE); //loadCameraImage();
		...
		cv::resize(cameraImage, cameraImage, cv::Size(cameraImage.cols * 0.5, cameraImage.rows * 0.5));
		...
		/*cv::Mat testImage = cv::imread("test.png", CV_LOAD_IMAGE_GRAYSCALE);
	
		cv::Mat homography2 = cv::Mat( 3, 3, CV_32FC1 );
		cv::FileStorage fw("homography.xml", cv::FileStorage::READ );
		fw["homography"] >> homography2; 
		fw.release();
		imshow( "Match", testImage );
		*/
	}
	return 0;
}
