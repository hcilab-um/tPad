...
namespace tPadNeuralNet
{
    ...
    public class NeuralNetwork
    {
        ...
        private static double HyperTanFunction(double x)
        {
            ...
            else return (1.0 - 2.0 / (Math.Exp(2.0 * x) + 1.0)); //faster than calling Math.tanh(x)
          ...
          double weightDecay)
        {
            ...
            double[] targetValues = new double[numOutput]; // target values
            ...
            while (epoch < maxEprochs)
            {
                ...
                for (int i = 0; i < trainData.Length; ++i)
                {
                    ...
                    Array.Copy(trainData[index], inputs, numInput);
                    Array.Copy(trainData[index], numInput, targetValues, 0, numOutput);
